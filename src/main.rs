//! FAT32 Shell - Interactive CLI
//!
//! Demo program to test the FAT32 filesystem implementation.
...
fn create_demo_image() -> Vec<u8> {
    ...
    data[fat_start + 8..fat_start + 12].copy_from_slice(&0x0FFFFFFFu32.to_le_bytes()); // Cluster 2 (root)
    data[fat_start + 12..fat_start + 16].copy_from_slice(&0x0FFFFFFFu32.to_le_bytes()); // Cluster 3 (DOCS)
    data[fat_start + 16..fat_start + 20].copy_from_slice(&0x0FFFFFFFu32.to_le_bytes()); // Cluster 4 (hello.txt)
    data[fat_start + 20..fat_start + 24].copy_from_slice(&0x0FFFFFFFu32.to_le_bytes()); // Cluster 5 (readme)
    data[fat_start + 24..fat_start + 28].copy_from_slice(&0x0FFFFFFFu32.to_le_bytes()); // Cluster 6 (info.txt)

    // Root directory (cluster 2, sector 64, offset 32768)
    let root_dir = 64 * 512;
    ...
    // DOCS directory content (cluster 3)
    let docs_dir = 65 * 512;
